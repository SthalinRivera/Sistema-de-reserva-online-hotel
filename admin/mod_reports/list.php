...
<div class="card card-primary">
  ...
  .titlereport{
    ...
    <section class="invoice">
      ...
      <div class="row ">
        <div class="col-xs-12">
          <h2 class="page-header titlereport">
            <i   class="fa fa-globe"></i>Hospedaje Pacífico
      ...
      <div class="col-sm-2 invoice-col">
        ...
        <div class="col-sm-2 invoice-col">
         Habitacion
          <address>
            <input class="form-control" size="20" type="text" value="<?php echo isset($_POST['txtsearch']) ? $_POST['txtsearch'] :'' ?>" Placeholder="Buscar por...." name="txtsearch" id="txtsearch">
        </address>    
        ...
        <option value="Llegada" <?php echo isset($_POST['categ']) && $_POST['categ'] == 'Arrival' ? 'selected' :'' ?>>Llegada</option>
        <option value="Pendientes" <?php echo isset($_POST['categ']) && $_POST['categ'] == 'Pending' ? 'selected' :'' ?>>Pendientes</option>
        <option value="Confirmado" <?php echo isset($_POST['categ']) && $_POST['categ'] == 'Confirmed' ? 'selected' :'' ?>>Confirmado</option>
			  </select>
		  </div>
          </address>
        </div>
        ...
        <div class="col-sm-2 invoice-col">
          ...
          <address> 
		  <div class="form-group">
			 <input class="form-control date start " size="20" type="date" value="<?php echo (isset($_POST['start'])) ? $_POST['start'] : date('y-m-d'); ?>" Placeholder="Check In" name="start" id="from" data-date=""  >
		 </div>
          </address>
        </div>
        <!-- /.col -->
        ...
        <address>
        <div class="form-group"> 
		      <input class="form-control date end " size="20" type="date" value="<?php echo (isset($_POST['end'])) ? $_POST['end'] : date('y-m-d'); ?>" Placeholder="Check Out"  name="end" id="end" data-date="" data-date-format="yyyy-mm-dd" data-link-field="any" data-link-format="yyyy-mm-dd">
		  </div>
      ...
      <div class="row">
        <div class="col-xs-12">
          <h2 class="page-header titlereport">
            ...
            <small class="pull-right"> <?php echo (isset($_POST['start'])) ? 'Registro llegada :' .$_POST['start'] : ''; ?> <?php echo (isset($_POST['end'])) ? ' Registro salida :' .$_POST['end'] : ''; ?> </small>
          </h2>
        </div>
        <!-- /.col -->
      </div>
  ...
  $mydb->setQuery($sql);
	...
	$cur = $mydb->loadResultList();
      ...
      <div class="row">
        ...
        <!-- /.col -->
        <div class="col-xs-3 titlereport">
          <p class="lead">Cantidad total</p>
...
<input type="hidden" name="txtsearch" value="<?php echo (isset($_POST['txtsearch'])) ? $_POST['txtsearch'] : ''; ?>">
 <input type="hidden" name="categ" value="<?php echo (isset($_POST['categ'])) ? $_POST['categ'] : ''; ?>">
    ...
    <input type="hidden" name="end" value="<?php echo (isset($_POST['end'])) ? $_POST['end'] :  date('Y-m-d'); ?>">
      ...
      <div class="row no-print titlereport">
        <div class="col-xs-12"> 
       <span class="pull-right"> <button type="submit" class="btn btn-primary"  ><i class="fa fa-print"></i>Imprimir</button></span>  
          </div>
      </div>
    </section>
    </form>
